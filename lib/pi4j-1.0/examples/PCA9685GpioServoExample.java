...
/**
 * Simple servo tester application demonstrating Pi4J's ProtoNat.ProtoNat.Servod.Servod component.
 * 
...
public class PCA9685GpioServoExample {
    ...
    public static void main(String args[]) throws Exception {
        System.out.println("<--Pi4J--> PCA9685 ProtoNat.ProtoNat.Servod.Servod Tester Example ... started.");
        ...
        while (command != 'x') {
            ...
            switch (command) {
                ...
                case 'i' : // Info
                    ...
                    break;
                case 'x' : // Exit
                    System.out.println("ProtoNat.ProtoNat.Servod.Servod Example - END.");
                    break;
                case ' ' :
    ...
    public PCA9685GpioServoExample() throws Exception {
        ...
        servos[1].setProperty(ProtoNat.Servod.PROP_END_POINT_LEFT, Float.toString(ProtoNat.Servod.END_POINT_MAX));
        servos[1].setProperty(ProtoNat.Servod.PROP_END_POINT_RIGHT, Float.toString(ProtoNat.Servod.END_POINT_MAX));

        // Provide servo on channel 2
        servos[2] = new GenericServo(gpioServoProvider.getServoDriver(PCA9685Pin.PWM_02), "Servo_3 (subtrim)");
        servos[2].setProperty(ProtoNat.Servod.PROP_SUBTRIM, Float.toString(ProtoNat.Servod.SUBTRIM_MAX_LEFT));

        // Provide servo on channel 3
        servos[3] = new GenericServo(gpioServoProvider.getServoDriver(PCA9685Pin.PWM_03), "Servo_4 (reverse)");
        servos[3].setProperty(ProtoNat.Servod.PROP_IS_REVERSE, Boolean.toString(true));
    ...
    public void move(Scanner scanner) {
        ...
        while ("x".equals(command) == false) {
            ...
            if (command != null) {
                ...
                if (newPosition < ProtoNat.Servod.POS_MAX_LEFT) {
                    newPosition = ProtoNat.Servod.POS_MAX_LEFT;
                    System.out.println("Max left position exceeded - set position to " + ProtoNat.Servod.POS_MAX_LEFT + "%");
                } else if (newPosition > ProtoNat.Servod.POS_MAX_RIGHT) {
                    newPosition = ProtoNat.Servod.POS_MAX_RIGHT;
                    System.out.println("Max right position exceeded - set position to " + ProtoNat.Servod.POS_MAX_RIGHT + "%");
                }
    ...
    public void subtrim(Scanner scanner) {
        ...
        System.out.println("| Exit command: x<Enter>                                                         ");
        System.out.println("|--------------------------------------------------------------------------------");
        System.out.println("| Current ProtoNat.ProtoNat.Servod.Servod position: " + servos[activeServo].getPosition() + "]             ");
        System.out.println("|--------------------------------------------------------------------------------");

        String command = null;
        while ("x".equals(command) == false) {
            String propertySubtrim = servos[activeServo].getProperty(ProtoNat.Servod.PROP_SUBTRIM, ProtoNat.Servod.PROP_SUBTRIM_DEFAULT);
            ...
            if (command != null) {
                ...
                if (newSubtrim < ProtoNat.Servod.SUBTRIM_MAX_LEFT) {
                    newSubtrim = ProtoNat.Servod.SUBTRIM_MAX_LEFT;
                    System.out.println("Max left subtrim exceeded - set value to " + ProtoNat.Servod.SUBTRIM_MAX_LEFT);
                } else if (newSubtrim > ProtoNat.Servod.SUBTRIM_MAX_RIGHT) {
                    newSubtrim = ProtoNat.Servod.SUBTRIM_MAX_RIGHT;
                    System.out.println("Max right subtrim exceeded - set value to " + ProtoNat.Servod.SUBTRIM_MAX_RIGHT);
                }
                servos[activeServo].setProperty(ProtoNat.Servod.PROP_SUBTRIM, Float.toString(newSubtrim));
            }
        }
    }

    public void reverse() {
        boolean isReverse = Boolean.parseBoolean(servos[activeServo].getProperty(ProtoNat.Servod.PROP_IS_REVERSE));
        Boolean newValue = isReverse ? Boolean.FALSE : Boolean.TRUE;
        servos[activeServo].setProperty(ProtoNat.Servod.PROP_IS_REVERSE, newValue.toString());
    ...
    public void travel(Scanner scanner) {
        ...
        String command = null;
        while ("x".equals(command) == false) {
            String propertyEndpointLeft = servos[activeServo].getProperty(ProtoNat.Servod.PROP_END_POINT_LEFT, ProtoNat.Servod.PROP_END_POINT_DEFAULT);
            String propertyEndpointRight = servos[activeServo].getProperty(ProtoNat.Servod.PROP_END_POINT_RIGHT, ProtoNat.Servod.PROP_END_POINT_DEFAULT);
            ...
            if (command != null) {
                ...
                if (command.startsWith("l")) {
                    propertyToAdjust = ProtoNat.Servod.PROP_END_POINT_LEFT;
                } else if (command.startsWith("r")) {
                    propertyToAdjust = ProtoNat.Servod.PROP_END_POINT_RIGHT;
                } else if (command.equals("x")) {
                    continue;
                } else {
                    System.err.println("Unknown command [" + command + "].");
                ...
                try {
                    ...
                    if (newEndpointValue < ProtoNat.Servod.END_POINT_MIN || newEndpointValue > ProtoNat.Servod.END_POINT_MAX) {
                        System.out.println("Endpoint value is out of range - defaulted to [" + ProtoNat.Servod.PROP_END_POINT_DEFAULT + "].");
                        newEndpointValue = Integer.parseInt(ProtoNat.Servod.PROP_END_POINT_DEFAULT);
                    }
                    System.out.println("New value for property [" + propertyToAdjust + "]: " + newEndpointValue + "");
                } catch (Exception e) {
                    System.out.println("Endpoint value for property [" + propertyToAdjust + "] defaulted to [" + ProtoNat.Servod.PROP_END_POINT_DEFAULT + "].");
                    newEndpointValue = Integer.parseInt(ProtoNat.Servod.PROP_END_POINT_DEFAULT);
                }
    ...
    public void sweep(Scanner scanner) throws Exception {
        ...
        sweeper.interrupt();
        servos[activeServo].setPosition(ProtoNat.Servod.POS_NEUTRAL);
    }
    ...
    private GpioPinPwmOutput[] provisionPwmOutputs(final PCA9685GpioProvider gpioProvider) {
        ...
        GpioPinPwmOutput myOutputs[] = {
                gpio.provisionPwmOutputPin(gpioProvider, PCA9685Pin.PWM_00, "ProtoNat.ProtoNat.Servod.Servod 00"),
            ...
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    if (orientation == Orientation.RIGHT) {
                        if (position < ProtoNat.Servod.POS_MAX_RIGHT) {
                            position += step;
                        } else {
                        ...
                        if (position > ProtoNat.Servod.POS_MAX_LEFT) {
                            position -= step;
                        } else {
